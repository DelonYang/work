...
func (j *Job) AddFuncWithRetry(topic string, f func(task Task) TaskResult, retry_max_num int, args ...interface{}) error {
	//worker并发数
	var concurrency int
	var extraParam []interface{}
	if len(args) > 0 {
	...
	w := &Worker{Call: MyWorkerFunc(f), MaxConcurrency: concurrency, ExtraParam: extraParam, AutoRetryMaxNum: retry_max_num}
	return j.AddWorker(topic, w)
}
